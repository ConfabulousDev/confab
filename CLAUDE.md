...
- The TS plugin (`pkg/provider/plugins/confab-sync.ts`) bridges lifecycle (the daemon does the data sync): on `session.created` it fires `confab hook session-start --provider opencode` with `{session_id, cwd}` (plus `parent_id` for subagents); on `dispose` it fires `confab hook session-end --provider opencode` for each session it started, signalling a final sync + shutdown. `cmd/hook_sessionend.go` routes `--provider opencode` to `sessionEndOpencode`, which calls `daemon.StopDaemonForProvider`.
