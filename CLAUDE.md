...
- `AnnotateChunk` is a no-op (the backend reads tokens/cost/model from each line; no `opencode_rollouts` table). `ScanSessions`/`FindSessionByID` are unsupported (live-sync only; offline manual mode deferred). Redaction applies automatically via `FileTracker.ReadChunk`, same as other providers.
