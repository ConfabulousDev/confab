...
- The TS plugin (`pkg/provider/plugins/confab-sync.ts`) fires `session.created` → `confab hook session-start --provider opencode` with `{session_id, cwd}` (plus `parent_id` for subagents). Lifecycle only — the daemon does the data sync.
...
- Daemon shutdown is **parent-PID driven** (the OpenCode CLI is the parent process). The collector retries on the DB indefinitely (file missing, locked, etc. all flow through the Warn cadence), so a long-lived daemon outlives transient DB-availability blips.
