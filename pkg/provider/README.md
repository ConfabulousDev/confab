...
| `opencode_db.go` | `OpenCodeDBReader` — reads OpenCode's local SQLite DB (`~/.local/share/opencode/opencode.db`, or `CONFAB_OPENCODE_DB` / `$XDG_DATA_HOME/opencode/opencode.db`). Each `ReadSession(ctx, sessionID, sinceMessageID)` call opens the DB read-only (`?mode=ro&_pragma=busy_timeout(5000)`), runs a single `LEFT JOIN message + part` indexed by `message(session_id, time_created, id)` + `part(message_id, id)`, and returns `[]ocRawEnvelope` with `id`/`sessionID`/`messageID` injected from row columns into the embedded JSON (those keys are never in `data` — the backend types require them). HWM (`m.id > sinceMessageID`) makes the query incremental so long sessions don't re-fetch the whole history every poll. Replaces the deleted `opencode_client.go` / `OpenCodeClient` HTTP/SSE path (CF-543). |
